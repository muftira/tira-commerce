...
function Shirts(props) {
  const shirtsData = JSON.parse(localStorage.getItem("shirts"))
  ...
  const handleClick = (e, list) => {
    e.preventDefault()
    const selected = shirtsData.filter(product => product.id === list.id ? list : null)
  ...
  return (
    <div className="w-full h-[calc(100vh-64px)] flex flex-col justify-start items-center">
        <p className='text-4xl font-bold mt-6'>T-Shirt</p>
        <div className="grid sm:grid-cols-3 grid-cols-1 lg:gap-8 gap-4 my-10">
          {shirtsData && shirtsData.map(item =><div onClick={(e) => handleClick(e, item) } key={item.id} className="lg:h-[350px] lg:w-64 h-[250px] w-[200px]  shadow-2xl border-2 border-neutral-300 rounded-xl overflow-hidden hover:cursor-pointer">
            <div className="lg:h-64 lg:w-64 h-[190px] w-[200px] bg-neutral-300  overflow-hidden">
            <img className='' src={item.color[0].pict[0].url} alt="" />
            </div>
...
export default connect(state => state)(Shirts)