...
function Pants(props) {
  const pantsData = JSON.parse(localStorage.getItem("pants"))
  ...
  const handleClick = (e, list) => {
    e.preventDefault()
    const selected = pantsData.filter(product => product.id === list.id ? list : null)
...
console.log("reducers", props);
  return (
    ...
    <p className='text-4xl font-bold mt-6'>Pants</p>
    <div className="grid sm:grid-cols-3 grid-cols-1 lg:gap-8 gap-4 my-10">
      {pantsData && pantsData.map(item =><div onClick={(e) => handleClick(e, item)} key={item.id} className="lg:h-[350px] lg:w-64 h-[250px] w-[200px]  shadow-2xl border-2 border-neutral-300 rounded-xl overflow-hidden hover:cursor-pointer">
...
export default connect(state => state)(Pants)